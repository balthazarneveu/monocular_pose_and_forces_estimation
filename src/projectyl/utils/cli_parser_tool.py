...
def add_video_parser_args(parser: argparse.Namespace, allow_global_trim=False) -> None:
    video_args = parser.add_argument_group("input video")
    video_args.add_argument("-global-trim", "--trim", nargs="+", type=float, help="Trim in seconds like"
                            "-t 4.8 5.3 or -t 0.5")
